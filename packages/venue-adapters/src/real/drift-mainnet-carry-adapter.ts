...
const DRIFT_MAINNET_CARRY_CONNECTOR_TYPE = 'drift_native_mainnet_execution';
const DEFAULT_COMMITMENT: Commitment = 'confirmed';
const DEFAULT_VENUE_ID = 'drift-solana-mainnet-carry';
const DEFAULT_VENUE_NAME = 'Drift Solana Mainnet Carry';
...
const SUPPORTED_EXECUTION_SCOPE = [
  ...
  'production execution with real funds',
] as const;
...
interface DriftMainnetCarryAdapterDependencies {
...
export interface DriftMainnetCarryAdapterConfig {
  venueId?: string;
  venueName?: string;
  rpcEndpoint: string;
  driftEnv?: DriftEnv;
  ...
  /**
   * When true, allows execution on mainnet. This is a safety flag that must be
...
function toBn(value: string, precision: BN, label: string): BN {
  ...
  if (!decimal.isFinite() || decimal.lte(0)) {
    throw new Error(`Drift mainnet execution requires a positive ${label}.`);
  }
...
function createExecutionSourceMetadata(commitment: Commitment): VenueTruthSourceMetadata {
  return {
    sourceKind: 'adapter',
    sourceName: DRIFT_MAINNET_CARRY_CONNECTOR_TYPE,
    connectorDepth: 'execution_capable',
    commitment,
    observedScope: ['cluster_version'],
    provenanceNotes: [
      'This connector can submit real Drift mainnet orders only for the narrow execution scope declared in its capability metadata.',
      ...
      'Mainnet execution requires explicit mainnetExecutionEnabled flag and proper environment configuration.',
    ],
  };
}
...
function resolveExecutionIdentity(config: DriftMainnetCarryAdapterConfig): ExecutionIdentityResolution {
...
export class DriftMainnetCarryAdapter implements VenueAdapter {
  ...
  private readonly dependencies: DriftMainnetCarryAdapterDependencies;
  private readonly mainnetExecutionEnabled: boolean;
  ...
  constructor(
    private readonly config: DriftMainnetCarryAdapterConfig,
    dependencies: DriftMainnetCarryAdapterDependencies = {},
  ) {
    ...
    this.mainnetExecutionEnabled = config.mainnetExecutionEnabled ?? false;
    this.authRequirementsSummary = config.authRequirementsSummary ?? [
      'DRIFT_RPC_ENDPOINT (mainnet-beta RPC)',
      'DRIFT_EXECUTION_ENV=mainnet-beta',
      'DRIFT_PRIVATE_KEY',
      'DRIFT_EXECUTION_SUBACCOUNT_ID (optional)',
      'DRIFT_EXECUTION_ACCOUNT_LABEL (optional)',
      'DRIFT_MAINNET_EXECUTION_ENABLED=true',
    ];
    this.dependencies = dependencies;

    if (this.driftEnv !== 'mainnet-beta') {
      throw new Error('DriftMainnetCarryAdapter only supports Drift mainnet-beta.');
    }

    const market = configs['mainnet-beta'].PERP_MARKETS.find((candidate) => candidate.symbol === SUPPORTED_MARKET_SYMBOL);
    if (market === undefined) {
      throw new Error(`Drift mainnet market ${SUPPORTED_MARKET_SYMBOL} was not found in the installed SDK.`);
    }
  ...
  async getCarryCapabilities(): Promise<CarryVenueCapabilities> {
    ...
    return {
      ...
      readOnly: false,
      approvedForLiveUse: this.mainnetExecutionEnabled && capability.missingPrerequisites.length === 0,
      sensitiveExecutionEligible: this.mainnetExecutionEnabled,
      promotionStatus: this.mainnetExecutionEnabled ? 'approved' : 'not_requested',
  ...
  async getVenueCapabilitySnapshot(): Promise<VenueCapabilitySnapshot> {
    ...
    if (!endpointConfigured) {
      missingPrerequisites.push(
        'Set DRIFT_RPC_ENDPOINT to a Solana mainnet-beta RPC endpoint before requesting promotion or execution.',
      );
    }

    if (!this.mainnetExecutionEnabled) {
      missingPrerequisites.push(
        'Mainnet execution is not enabled. Set DRIFT_MAINNET_EXECUTION_ENABLED=true to enable mainnet trading.',
      );
    }
    ...
    if (identity.error !== null) {
      ...
      missingPrerequisites.push(
        'Set DRIFT_PRIVATE_KEY to the Drift mainnet authority secret key to enable signed execution and account truth.',
      );
    }
    ...
    return {
      ...
      connectorType: DRIFT_MAINNET_CARRY_CONNECTOR_TYPE,
      truthMode: 'real',
      readOnlySupport: true,
      executionSupport: true,
      approvedForLiveUse: this.mainnetExecutionEnabled && missingPrerequisites.length === 0,
  ...
  async getVenueTruthSnapshot(): Promise<VenueTruthSnapshot> {
    ...
    if (!endpointConfigured) {
      return {
        venueId: this.venueId,
        venueName: this.venueName,
        snapshotType: 'drift_mainnet_execution_connectivity',
        snapshotSuccessful: false,
        healthy: false,
        healthState: 'unavailable',
        summary: 'Drift mainnet execution connector is unconfigured because no RPC endpoint is set.',
        ...
        payload: {
          ...
          endpointConfigured,
          mainnetExecutionEnabled: this.mainnetExecutionEnabled,
    ...
    try {
      ...
      if (identity.keypair === null || identity.accountAddress === null) {
        return {
          ...
          snapshotCompleteness: 'minimal',
          truthCoverage: {
            accountState: unsupportedCoverage('Configure DRIFT_PRIVATE_KEY to derive the Drift mainnet user account.'),
            balanceState: unsupportedCoverage('This connector does not expose generic wallet balances.'),
            capacityState: unsupportedCoverage('Carry connector does not expose treasury-style capacity.'),
      ...
      return {
        ...snapshot,
        snapshotType: 'drift_mainnet_execution_account',
        sourceMetadata: {
          ...
          provenanceNotes: [
            ...
            'Real execution remains restricted to BTC-PERP market orders on mainnet-beta.',
            'Promotion approval, current evidence eligibility, runtime live mode, and backend authorization all gate execution.',
            'Mainnet execution requires explicit DRIFT_MAINNET_EXECUTION_ENABLED=true flag.',
          ],
        },
        metadata: {
          ...snapshot.metadata,
      ...
      return {
        ...
        healthy: false,
        healthState: 'unavailable',
        summary: 'Drift mainnet execution connector failed to refresh venue truth.',
        errorMessage: message,
        capturedAt,
        snapshotCompleteness: 'minimal',
        truthCoverage: {
          accountState: unsupportedCoverage('Drift mainnet execution truth could not be captured because the connector failed before account state was loaded.'),
          balanceState: unsupportedCoverage('This connector does not expose generic wallet balances.'),
          capacityState: unsupportedCoverage('Carry connector does not expose treasury-style capacity.'),
  ...
  async placeOrder(params: PlaceOrderParams): Promise<PlaceOrderResult> {
    ...
    if (!this.mainnetExecutionEnabled) {
      throw new Error(
        'Drift mainnet execution is disabled. Set mainnetExecutionEnabled=true in config to enable.',
      );
    }
    ...
    if (reduceOnly) {
      if (activePosition === null) {
        throw new Error(
          'Drift mainnet reduce-only execution requires an existing BTC-PERP position, but no open BTC-PERP position was found.',
        );
      }
      ...
      if (
        ...
        throw new Error(
          `Drift mainnet reduce-only execution can only reduce the current ${activePosition.side} BTC-PERP position.`,
        );
      }
      ...
      throw new Error(
        `Drift mainnet execution cannot cross the current ${activePosition.side} BTC-PERP position with a non-reduce-only ${params.side} order; reduce that position first.`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Drift mainnet execution failed while submitting a BTC-PERP market order: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async cancelOrder(venueOrderId: string): Promise<CancelOrderResult> {
    ...
    return {
      ...
      reason: cachedOrder === undefined
        ...
        : 'Drift mainnet carry execution intentionally does not expose a cancel path for the supported BTC-PERP market-order scope.',
    };
  }
  ...
  private assertSupportedAsset(asset: string): void {
    if (asset.trim().toUpperCase() !== SUPPORTED_ASSET) {
      throw new Error(
        `Drift mainnet carry adapter only supports ${SUPPORTED_MARKET_SYMBOL} for this phase; received asset "${asset}".`,
      );
    }
  }
  ...
  private assertSupportedOrder(params: PlaceOrderParams): void {
    ...
    if (params.type !== 'market') {
      throw new Error('Drift mainnet execution only supports market orders in this phase.');
    }
    if (params.postOnly === true) {
      throw new Error('Drift mainnet execution does not support post-only orders in this phase.');
    }
    if (params.price !== undefined) {
      throw new Error('Drift mainnet execution does not accept explicit limit prices in this phase.');
    }
    if (
      ...
      throw new Error(
        `Drift mainnet execution is limited to ${this.supportedMarket.symbol}; the provided market identity does not match that scope.`,
      );
    }
  }
  ...
  private buildConnectorMetadata(identity: ExecutionIdentityResolution): Record<string, unknown> {
    return {
      ...
      executionPosture: 'mainnet_execution_capable',
      connectorMode: 'execution_capable_mainnet',
  ...
  private buildMarketIdentity(
    ...
    return createCanonicalMarketIdentity({
      ...
      capturedAtStage,
      source: DRIFT_MAINNET_CARRY_CONNECTOR_TYPE,
      notes: [
        'Market identity is fixed to the single BTC-PERP mainnet market supported by this connector.',
      ],
    });
  }
  ...
  private async getDriftClient(): Promise<DriftExecutionClient> {
    ...
    const client = createDriftClient({
      ...
      spotMarketIndexes: configs['mainnet-beta'].SPOT_MARKETS.map((market) => market.marketIndex),
      oracleInfos: [{
  ...
  private async loadMarketContext(): Promise<{
    ...
    if (market === undefined) {
      throw new Error(
        `Drift SDK did not return market state for ${this.supportedMarket.symbol} on mainnet-beta.`,
      );
    }
